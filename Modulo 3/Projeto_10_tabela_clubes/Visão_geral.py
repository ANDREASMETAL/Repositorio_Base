...
filtro = st.sidebar.multiselect('Ano', anos, anos)
df = df[df['Ano'].apply(lambda x: x in  filtro)]
...
with st.container():
    ...
    with col1:
        with st.container(border=True, height="stretch"):
            st.metric('QT Vitorias', df['Vitorias'].sum())

    with col2:
        with st.container(border=True, height="stretch"):
            st.metric('QT Derrotas', df['Derrotas'].sum())

    with col3:
        with st.container(border=True, height="stretch"):
            st.metric('QT Empates', df['Empates'].sum())

    with col4:
        with st.container(border=True, height="stretch"):
            st.metric('QT Estrangeiros', df['Estrangeiros'].sum())
...
with st.container():
    ...
    with col2:
        with st.container(height='stretch'):
            ...
            fig = px.line(df_aux, x='Ano', y='Valor_total', title='Valor total dos clubes por ano')
            st.plotly_chart(fig)
